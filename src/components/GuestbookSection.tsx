...
export default function GuestbookSection({ messages }: GuestbookSectionProps) {
  ...
  return (
    <section id="recados" className="py-20 md:py-32 px-4 sm:px-6 md:px-12 max-w-6xl mx-auto w-full border-b border-zinc-100">
      <div className="text-center space-y-4 mb-12 sm:mb-16">
        <span className="text-[10px] sm:text-xs tracking-widest font-sans text-brand-muted uppercase block">
          Mural de Carinho
        </span>
        <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-light text-zinc-900">
          Recados dos Convidados
        </h2>
        <p className="text-xs sm:text-sm text-zinc-500 font-light max-w-md mx-auto leading-relaxed">
          ...
          {[
            ...
            return (
              <button
                ...
                className={`px-4 py-2 text-xs font-sans tracking-wider uppercase font-semibold rounded-xl transition-all flex items-center gap-2 ${
              ...
              >
                ...
                <span className={`px-1.5 py-0.5 text-[10px] font-mono rounded-md ${
                  activeFilter === filter.id ? 'bg-white/20 text-white' : 'bg-zinc-200/70 text-zinc-600'
                }`}>
                  {filter.count}
                </span>
              </button>
      ...
      {displayedMessages.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayedMessages.map((msg) => (
            <div
              key={msg.id}
              className="bg-white border border-zinc-150 rounded-popyn p-6 md:p-8 flex flex-col justify-between shadow-xs hover:shadow-md transition-all hover:border-zinc-300 group"
            >
              <div className="space-y-4">
                ...
                <div className="relative">
                  <span className="absolute -top-3 -left-2 text-4xl font-serif text-zinc-150 select-none group-hover:text-zinc-200 transition-colors">“</span>
                  <p className="font-serif italic text-zinc-700 text-sm sm:text-base leading-relaxed pl-3 relative z-10 pt-1">
                    {msg.notes}
                  </p>
                </div>
              </div>
              ...
              <div className="pt-6 mt-6 border-t border-zinc-100 space-y-1">
                <span className="font-serif text-sm font-semibold text-zinc-950 block">
                  {msg.name}
                </span>
                <span className="text-[10px] font-sans tracking-wide text-zinc-450 uppercase flex items-center gap-1.5">
                  {msg.type === 'presence' ? (
                    <>
                    ...
                    <>
                      ...
                      Presenteou: <strong className="text-zinc-650 max-w-[150px] truncate">{msg.giftName}</strong>
                    </>
                  )}
                </span>
              </div>
        ...
        /* Empty State */
        <div className="text-center py-16 border border-dashed border-zinc-200 rounded-popyn max-w-xl mx-auto space-y-3 bg-zinc-50/50">
      ...
      {hasMore && (
        <div className="text-center pt-10">
          <button
            ...
            className="px-6 py-3 border border-zinc-200 hover:bg-zinc-50 text-zinc-850 font-sans text-xs font-semibold uppercase tracking-wider rounded-xl transition-all inline-flex items-center gap-2 active:scale-95"
          >
